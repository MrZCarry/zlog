...
class LogParser:
    ...
    def _abstract_dynamic_structural_tokens(self, template, log_list):
        """
        Ultra-fast optimized version: Sentinel interception at the first line.
        1. Sentinel check: Only take log_list[0], if the first line has no 'Number + Uppercase Unit', directly determine that the whole group does not involve this logic.
        2. Template validation: If the first line has a unit, check if the template also captures the unit.
        3. Span sampling: Only when both are hit, start sampling validation.
        """
    ...
    def _update_token_usage(self, prompt, generated_text):
        """Calculate and accumulate the total number of tokens for this LLM call."""
        try:
            prompt_str = str(prompt) if isinstance(prompt, list) else prompt
            
            p_tokens = len(self.tokenizer.encode(prompt_str))
            g_tokens = len(self.tokenizer.encode(generated_text))
            
            self.total_tokens += (p_tokens + g_tokens)
            self.total_calls += 1
        except Exception as e:
            pass

    def verify_markers_preserved(self, log_content, template_regex, dynamic_markers):
        """
        Verify whether the template preserves the key markers from the original log.
        Principle:
        1. Find the specific text matched by the Marker in the original log.
        2. Check if this specific text exists in the template.
        """
        clean_template = template_regex.replace(r'(.*?)', '<VAR>')
        clean_template = clean_template.replace(r'\s', ' ').replace(r'\.', '.').replace(r'\-', '-')

        for marker in dynamic_markers:
            match_in_log = re.search(re.escape(marker), log_content, re.IGNORECASE)
            if match_in_log:
                matched_text_in_log = match_in_log.group(0).strip()

                if len(matched_text_in_log) < 2:
                    continue

                if matched_text_in_log not in clean_template:
                    unescaped_template = clean_template.replace('\\', '')
                    if matched_text_in_log not in unescaped_template:
                        return False
        return True

    def _generate_correction_context(self, original_template, failed_log, dynamic_markers):
        """Generate a correction context containing the reason for the error to enhance the Plausible Template error prompt."""
        ...
        present_markers = []
        for marker in dynamic_markers:
            if re.search(rf'\b{re.escape(marker)}\b', failed_log):
                present_markers.append(marker)

        missing_markers = []
        for marker_text in present_markers:
            if marker_text not in original_template:
                missing_markers.append(marker_text)

        present_markers_info = []
        for marker in dynamic_markers:
            log_count = failed_log.count(marker)
            if log_count > 0:
                present_markers_info.append({
                    'marker': marker,
                    'log_count': log_count
                })

        missing_repetition_warnings = []
        for marker in dynamic_markers:
            lc = len(re.findall(rf'\b{re.escape(marker)}\b', failed_log))
            if lc <= 0: continue  

            tc = len(re.findall(rf'\b{re.escape(marker)}\b', pseudo_template))
            if lc - tc > 0:
                missing_repetition_warnings.append(marker)
    ...
    """
        ...
        if missing_markers:
            correction_msg += "[FATAL] CONSTANT KEYWORD ERROR.\n"
            correction_msg += "You failed to preserve required constant keywords. Based on the definition, the following words appearing in the log MUST ALL be kept as static text:\n"

            for m in present_markers:
                if m in missing_markers:
                    correction_msg += f"   - {m} -> [MISSING! Restore this word]\n"
                else:
                    correction_msg += f"   - {m} -> [Correct. Keep it]\n"

            correction_msg += "ACTION: Re-generate the template. MUST Ensure ALL the above keywords are present exactly as shown.\n"
            correction_msg += "CRITICAL INSTRUCTION: If you cannot find any parts that actually vary, it is perfectly fine to output a template with NO dynamic variables (<*>). \n"
            correction_msg += "PRIORITY #1 is to PRESERVE these static markers. Do NOT abstract them just to create a variable.\n"
        ...
        if missing_repetition_warnings:
            positions = []
            for m in missing_repetition_warnings:
                pos = failed_log.find(m)
                if pos != -1:
                    positions.append((pos, m))

            sorted_markers = [item[1] for item in sorted(positions)]

            marker_structure = "".join([f"{{{m}}}" for m in sorted_markers])

            correction_msg += f"\n[TRUNCATION ERROR] Detected a repeating pattern that was omitted.\n"
            correction_msg += f"The log contains a repeating sequence of {marker_structure}.\n"
            correction_msg += f"The original log contains a long sequence of repeating static markers that you MUST write out ENTIRELY.\n"
            correction_msg += f"NEVER use '...' or any placeholder to shorten the template. You must match the FULL content of the log from start to end.\n"

            for m in sorted_markers:
                actual = len(re.findall(rf'\b{re.escape(m)}\b', failed_log))
                provided = len(re.findall(rf'\b{re.escape(m)}\b', pseudo_template))
                correction_msg += f"   - '{m}' appears {actual} times in the log, but you only matched {provided}.\n"

            correction_msg += f"ACTION: You MUST NOT shorten the template. Repeat the {marker_structure} structure "
            max_actual_counts = max(
                [len(re.findall(rf'\b{re.escape(m)}\b', failed_log)) for m in sorted_markers]) if sorted_markers else 0
            correction_msg += f"exactly {max_actual_counts} times to match the whole log.\n"
    ...
    def detect_coverage_issues(self, log, regex):
        """
        Detect whether the template covers the complete log content.
        Adopt a 'strict first, then loose' strategy to resolve Lazy Quantifier false positives and real truncation detection.
        """
    ...
    def _get_structural_representation(self, text):
        """Generate a structural representation of the text, highlighting the positions of constants and variables, and strengthening symbol comparison."""
    ...
    def _inject_correction_into_prompt(self, base_prompt, correction_context, is_chatglm=False, is_mistral=False, is_qwen=False):
        """Inject the correction information into the base prompt."""
        if is_chatglm:
    ...
    def generate_prompt_with_log_list(self, log_list, dic=False, correction_context=None, dynamic_markers=None):
            trimmed_list_log = self.adaptive_random_sampling(log_list, self.regex_sample, dic=dic)
            present_markers = dynamic_markers if dynamic_markers else []
            proactive_tips = ""
    ...
    OBSERVATION: The following words appear identically in ALL provided sample logs: {', '.join(present_markers) if present_markers else 'none'}You must keep them as static text.
    {proactive_tips}
    ...
    Print the input logs’ template delimited by backticks"""
            ...
            message = [
                ...
                {
                    "role": "assistant",
                    "content": "`service_check(sshd:auth): user <*>; status ok.`",
                },
                {
                    "role": "user",
                ...
                {
                    "role": "user",
                    "content": 'Log list: ["ipv4 Connection failed : #1 [mail.net-com.cn] 1.1-2.3 (abcd-75-66.pacbell.baidu.net) ...", "ipv4 Connection failed : #1 [mail.net-com.cn] 1.1-2.3 (abcd-75-66.pacbell.baidu.net) ..."]',
                },
                {
                    "role": "assistant",
                    "content": "`ipv4 Connection failed : <*> [<*>] <*> (<*>) ...`",
                },
                {
                    "role": "user",
...
Print the input logs' template delimited by backticks."""
    ...
    def generate_prompt_with_log_list_chatglm(self, log_list, dic=False, correction_context=None, dynamic_markers=None):
        trimmed_list_log = self.adaptive_random_sampling(log_list, self.regex_sample, dic=dic)
        messages = [
            {
                "role": "system",
    ...
    def generate_prompt_with_log_list_mistral(self, log_list, dic=False, correction_context=None, dynamic_markers=None):
        trimmed_list_log = self.adaptive_random_sampling(log_list, self.regex_sample, dic=dic)
        messages = [
            {
                "role": "user",
    ...
    def template_to_regex(self, template):
        ...
        print(f"DEBUG template_to_regex[end]: {template}")
    ...
    def correct_single_template(self, template, log=None):
        print(f"DEBUG [CS-Start]: {template}")
        ...
        print(f"DEBUG [CS-AfterTokenJoin]: {template}")
        ...
        if log and '(.*?)' in template:
            ...
            try:
                ...
                log_match = re.match(full_regex, log)
                if log_match:
                    ...
                    for m in reversed(candidates):
                        ...
                        if token1_val and token1_val.isalpha():
                            ...
                            print(f"DEBUG [CS-Restore]: Successfully restored '{token1_val}'")
            except Exception as e:
                print(f"DEBUG [CS-Restore] Error: {e}")
    ...
    def check_and_modify_regex(self, regex, string):
        """
        Core reflection mechanism (Plan A): Check the variable content matched by the regex, and fix the static symbols swallowed by variables.
        If the regex structure is modified (capture group count changes), regenerate the match object immediately to ensure index synchronization.
        """
    ...
    def clean_regex(self, log, regex):
        print(f"DEBUG clean_regex[start]: {regex}")
        regex = regex.strip()
        if log:
        ...
        if log:
            ...
            for _ in range(max_iterations):
                ...
                if regex != old_regex_iteration:
                    print(f"DEBUG clean_regex[Fill symbols]: {regex}")
                ...
                if indices_to_remove:
                    ...
                    print(f"DEBUG clean_regex[Delete symbols]: {regex}")
                if regex == old_regex_iteration:
                    break
        ...
        '''
        if '(.*?)' in regex:
            chunks = re.split(r'(\(\.\*\?\))', regex)
            if len(chunks) > 2:
                refined_chunks = []
                i = 0
                while i < len(chunks):
                    current_item = chunks[i]

                    if current_item == '(.*?)' and i > 0 and i < len(chunks) - 1:
                        prev_text = chunks[i - 1]
                        next_text = chunks[i + 1]

                        if prev_text.rstrip().endswith('='):
                            refined_chunks.append(current_item)
                            i += 1
                            continue

                        prev_word_match = re.search(r'([a-zA-Z0-9]+)\s*$', prev_text)
                        next_word_match = re.search(r'^\s*([a-zA-Z0-9]+)', next_text)

                        if prev_word_match and next_word_match:
                            w1 = prev_word_match.group(1)
                            w2 = next_word_match.group(1)

                            if re.search(rf'\b{re.escape(w1)}\s+{re.escape(w2)}\b', log):
                                i += 1
                                continue

                    refined_chunks.append(current_item)
                    i += 1
                regex = "".join(refined_chunks)
        '''
        if log and "..." in log:
            if " ..." in log:
                if r"\.\.\." in regex and r" \.\.\." not in regex:
                    ...
                    regex = regex.replace("...", " ...")
        regex = regex.replace("<*>", "").replace("()", "")
        print(f"DEBUG clean_regex[end0]: {regex}")
        return regex
        ...
        try:
            if "chatglm" in self.model:
                messages, sampled_log_list = self.generate_prompt_with_log_list_chatglm(
                    log_list, dic=dic, correction_context=correction_context, dynamic_markers=dynamic_markers
                )
                response, history = self.pipeline[0].chat(self.pipeline[1], messages[-1]["content"],
                                                          history=messages[:-1], do_sample=do_sample)
                self._update_token_usage(messages, response)
                ...
                vocab = self.pipeline.tokenizer.get_vocab()
                if "<|im_end|>" in vocab:
                    terminators.append(self.pipeline.tokenizer.convert_tokens_to_ids("<|im_end|>"))
                if "<|endoftext|>" in vocab:
                    terminators.append(self.pipeline.tokenizer.convert_tokens_to_ids("<|endoftext|>"))
                terminators = [t for t in terminators if t is not None]
                safe_pad_token_id = self.pipeline.tokenizer.pad_token_id or self.pipeline.tokenizer.eos_token_id or 151643  
                ...
                self._update_token_usage(prompt, raw_out)
                ...
                print(f"Raw model output (Llama): {out}")
                ...
                self._update_token_usage(prompt, out)
                return resul, out
            ...
            if "chatglm" in self.model:
                ...
                self._update_token_usage(prompt, out)
                return self.clean_regex(sampled_log_list[0],
                                        self.template_to_regex(outputs[0]["generated_text"][len(prompt):])), out
    ...
    def _get_proactive_instructions(self, sample_logs):
        """
        Analyze log features beforehand, utilizing existing detection logic to generate targeted prompts.
        """
        instructions = []
    ...
    def _detect_parameters_in_raw_log(self, log_content):
        """
        [Pre-prompt exclusive] Scan the raw log for 'key=value' parameters that need abstraction.
        Distinctions from detect_hardcoded_parameters:
        1. Does not process (.*?) placeholders.
        2. Does not remove escape characters (preserves the original log appearance).
        3. Only used to determine whether to prompt the model for parameter abstraction.
        """
    ...
    def detect_hardcoded_parameters(self, template_regex):
        """
        Detect if there are hardcoded parameters in the template (e.g., tty=ssh, uid=0, url=http://...).
        Principle: If 'key=value' appears and the value is a specific word instead of (.*?), it is considered hardcoded.
        """
        clean_template = template_regex.replace(r'(.*?)', '<VAR>')
    ...
    def check_regex_from_groups(self, res_list, groups_dict_list, log_regex, new_event=0, history_str=None,
        ...
        template_marker_counts = {}
        has_hardcoded_errors = False
        hardcoded_errors_msg = None

        if self.do_self_reflection == "True" and cleaned_candidate:
            if dynamic_markers:
                pseudo_temp_for_count = cleaned_candidate.replace(r'(.*?)', '<*>')
                for marker in dynamic_markers:
                    template_marker_counts[marker] = pseudo_temp_for_count.count(marker)
        ...
        if cleaned_candidate and not has_hardcoded_errors:
            ...
            for content in sampled_contents:
                ...
                if is_match and self.do_self_reflection == "True" and dynamic_markers:
                    if not self.verify_markers_preserved(content, cleaned_candidate, dynamic_markers):
                        is_match = False
                    if is_match:
                        for marker in dynamic_markers:
                            if content.count(marker) - template_marker_counts.get(marker, 0) > 0:
                                is_match = False
                                break
    ...
    def store_regx_for_logs(self, res_list, groups_dict_list, log_regex, initial_raw_output="", dynamic_markers=None):
        ...
        res_list, wrong_logs = self.check_regex_from_groups(
            res_list, groups_dict_list, log_regex, history_str=history_str, dynamic_markers=dynamic_markers
        )
        ...
        while len(wrong_logs) > 0 and (test_time < max_retries and len_wrong == len(wrong_logs)):
            ...
            res_list, wrong_logs = self.check_regex_from_groups(
                res_list, wrong_logs, log_regex, self.new_event, history_str=history_str, dynamic_markers=dynamic_markers
            )
    ...
    def parse(self, groups_from_parser, logs):
        ...
        current_group_markers = self.marker_extractor.get_static_markers(log_contents)
        ...
        if matched_template:
            clean_template = matched_template.replace(r'\(.*?\)', '<*>').replace('\\', '')
            is_cache_valid = all(m in clean_template for m in current_group_markers)

            if is_cache_valid:
                print(f"✅ Group representative hit and passed validation: {matched_template[:50]}...")
                for log in logs:
                    if verify_one_regex(log["Content"], matched_template):
                        res_list.append((log["LineId"], log["Content"], "0", matched_template, "Group Cache Matched"))
                    else:
                        self.unmatched_logs.append((log, matched_template))

                if len(res_list) == original_log_count:
                    self.total_time += (datetime.now() - start_time).total_seconds()
                    return res_list
            else:
                print(f"⚠️ Cache hit but semantic validation failed: template missing static words {current_group_markers}")

        matched_line_ids = {r[0] for r in res_list}
        logs = [l for l in logs if l["LineId"] not in matched_line_ids]
        log_contents_full = [log["Content"] for log in logs]
        MAX_DETECTION_SAMPLES = 500
        if len(log_contents_full) > MAX_DETECTION_SAMPLES:
            detection_samples = random.sample(log_contents_full, MAX_DETECTION_SAMPLES)
            print(f"DEBUG: Sampled {MAX_DETECTION_SAMPLES} logs for split detection")
        else:
            detection_samples = log_contents_full

        split_result = self.marker_extractor.detect_binary_split_with_bert(
            detection_samples,
            bert_threshold=0.14
        )

        if split_result:
            print(f"Split Detected: {list(split_result.keys())}")
            final_split_groups = {}
            split_keys = [k for k in split_result.keys() if k not in ["<NO_ED>", "<UNKNOWN>", "<OTHER_BRANCHES>"]]
            ...
            for log_obj in logs:
                ...
                for k in split_keys:
                    if k in content:  
                        final_split_groups.setdefault(k, []).append(log_obj)
                        assigned = True
                        break
                if not assigned:
                    final_split_groups.setdefault("<NO_ED>", []).append(log_obj)
            for token_key, sub_logs_struct in final_split_groups.items():
                if not sub_logs_struct:
                    continue
                sub_log_contents = [l["Content"] for l in sub_logs_struct]
                sub_dynamic_markers = self.marker_extractor.get_static_markers(sub_log_contents)
                if token_key != "<NO_ED>":
                    clean_key = token_key.strip()
                    if clean_key and clean_key not in sub_dynamic_markers:
                        if any(clean_key in log for log in sub_log_contents[:5]):
                            sub_dynamic_markers.append(clean_key)
                log_regex, raw_out = self.generate_log_template_using_pipeline(
                    log_list=sub_logs_struct,
                    dic=True,
                    dynamic_markers=sub_dynamic_markers,
                    force_llm=True,
                )

                res_list = self.store_regx_for_logs(
                    res_list, sub_logs_struct, log_regex,
                    initial_raw_output=raw_out, dynamic_markers=sub_dynamic_markers,
                )

                processed_line_ids = {log["LineId"] for log in sub_logs_struct}
                groups_from_parser = [g for g in groups_from_parser if g.get("LineId") not in processed_line_ids]

        else:
            dynamic_markers = current_group_markers
            ...
            if self.check_pre_logs(log_list=logs, dic=True):
                ...
                if any_sample_matched:
                    ...
                    for log in logs.copy():
                        ...
                        if matched_regex:
                            clean_m = matched_regex.replace(r'\(.*?\)', '<*>').replace('\\', '')
                            if all(m in clean_m for m in dynamic_markers):
                                matched_logs.append(log)
                                res_list.append((log["LineId"], content, "0", matched_regex,
                                                 "Matched Cache: " + matched_regex))
                                last_matched_regex = matched_regex	
                else:
                    print(f"DEBUG: Group pre-check failed (0 hits in 100 samples), skipping cache matching for {len(logs)} logs, proceeding directly to LLM parsing.")
        ...
        if len(res_list) != original_log_count:
            print(f"Warning: The number of parsed results ({len(res_list)}) does not match the number of input logs ({original_log_count})!")
