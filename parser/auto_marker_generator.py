...
class UnsupervisedMarkerExtractor:
    def __init__(self, model_path='roberta-base'):
        ...
        self.scoring_records = []
        try:
            self.tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=True)
            self.model = RobertaForMaskedLM.from_pretrained(model_path).to(self.device)
            self.model.eval()
            if not self.tokenizer.is_fast:
                print("⚠️ Warning: A Slow Tokenizer is loaded, offset_mapping might not work!", flush=True)
            else:
    ...
    def _is_valid_static_ed_word(self, token, log_content):
        """
        Determine if a token meets the criteria for a highly static -ed word.
        """
    ...
    def _get_context_signature(self, log_text, split_token):
        """
        Extract the context signature of a log:
        1. Remove the split_token.
        2. Keep only the remaining pure English letter words (excluding numbers and symbols).
        3. Join them into a string to act as the signature.
        """
    ...
    def _smart_sampling(self, log_group, k=30, min_threshold=0.6):
        """
        Optimized smart sampling: Prefer to keep 'different' logs rather than 'most similar' ones.
        """
        n = len(log_group)
        if n <= k:
            return log_group
    ...
    def _calculate_compound_score(self, log_text, candidate_word):
        """
        Fixed V5 (BugFix): Fixed TypeError where a list was added to a float.
        """
        try:
    ...
    def _needs_bert_optimization(self, common_tokens, representative_log, sample_logs):
        """
        Determine if the log group has Apache/Zookeeper/OpenSSH characteristics.
        If it does, return True (enable BERT); otherwise, return False (skip BERT).
        """
    ...
    def _is_valid_double_colon_static(self, token, log_content):
        """
        Determine if a token is a static token2 in a token1::token2 structure.
        Logic:
        1. Allow one of the identifiers in a pair to contain numbers (e.g., version numbers).
        2. If both in a pair contain numbers, both are considered dynamic variables.
        3. Attribute contagion: If a word is marked dynamic, or connected to one, the entire chain is considered dynamic.
        """
        pairs = []
    ...
    def _is_wrapped(self, text, index):
        """
        Check if the token at the specified position is wrapped in single quotes or brackets
        """
        before = text[:index]
        if before.count("'") % 2 != 0:
            return True
    ...
    def get_static_markers_core(self, log_group, confidence_threshold=0.1):
        ...
        for token in common_tokens:
            ...
            '''
                if token_idx != -1:
                    pre_content = representative_log[:token_idx]
                    pre_parts = pre_content.rstrip().split()
                    if pre_parts:
                        # Improvement: Use regex to extract the purely numerical part, supporting decimals
                        last_segment = pre_parts[-1]
                        # After removing interference characters like brackets, check if it's a number (including decimals)
                        clean_num = re.sub(r'[^\d.]', '', last_segment)

                        #Check if clean_num is a valid integer or float
                        if re.match(r'^\d+(\.\d+)?$', clean_num):
                            print(
                                f"[Lexical Skip] Token: [{token}] is a bracketed unit after number {clean_num}. Result: ❌ DROP")
                            continue
            '''
            ...
            if lower_clean.endswith('ed'):
                # 1. Internal format interception: exclude containing _ or -, exclude pure uppercase, limit length (5-14 characters)
                if '_' in token or '-' in token:
                    is_intercepted = True
                elif token.isupper():
                    is_intercepted = True
                elif not (4 < len(clean_token) < 15):
                    is_intercepted = True

                if not is_intercepted:
                    t_idx = representative_log.find(token)
                    if t_idx != -1:
                        pre_text = representative_log[:t_idx]
                        post_text = representative_log[t_idx + len(token):]

                        # 2. Intercept token followed immediately by - (e.g.: named -t)
                        after_token_text = post_text.lstrip()
                        if after_token_text.startswith('-'):
                            is_intercepted = True

                        # 3. Intercept tokens wrapped by [], '', "", (), {}
                        if not is_intercepted:
                            pairs = [('[', ']'), ("'", "'"), ('"', '"'), ('(', ')'), ('{', '}')]
                            for open_b, close_b in pairs:
                                last_open = pre_text.rfind(open_b)
                                if last_open != -1:
                                    # Ensure no close truncates between open and token
                                    if close_b not in pre_text[last_open:]:
                                        first_close = post_text.find(close_b)
                                        if first_close != -1:
                                            # Ensure no new open starts between token and close
                                            if open_b not in post_text[:first_close]:
                                                is_intercepted = True
                                                break

                        # 4. Intercept words after a colon (within 5 tokens) or equals sign (within 3 tokens)
                        if not is_intercepted:
                            # Determine distance to colon
                            last_colon = pre_text.rfind(':')
                            if last_colon != -1:
                                words_after_colon = re.findall(r'\w+', pre_text[last_colon:])
                                if len(words_after_colon) <= 5:
                                    is_intercepted = True

                            # Determine distance to equals sign
                            if not is_intercepted:
                                last_eq = pre_text.rfind('=')
                                if last_eq != -1:
                                    words_after_eq = re.findall(r'\w+', pre_text[last_eq:])
                                    if len(words_after_eq) <= 3:
                                        is_intercepted = True
            '''
            if self._is_valid_static_ed_word(token, representative_log):
                current_threshold = confidence_threshold * 0.001
                threshold_reason = "Suffix (ed) - High Priority"
            ...
            if self._is_valid_static_ty_word(token, representative_log):
                ...
                threshold_reason = "Suffix (ty) - High Priority"
            if lower_clean.endswith(('ful', 'lly')):
                current_threshold = 0.0
                threshold_reason = "Suffix (ful/lly) - Force Keep"
            ...
            if is_temporal_entity:
                ...
                current_threshold = confidence_threshold * 2.6
            ...
            if self._is_valid_double_colon_static(token, representative_log):
                current_threshold = confidence_threshold * 0.0001
                threshold_reason = "Double Colon (::) - High Priority"
            ...
            self.scoring_records.append({
                'log_content': representative_log,
                'token': clean_token,
                'passed': score > current_threshold,
                'score': score
            })
